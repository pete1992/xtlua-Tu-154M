...
{
	if(!callback || callback->L == nullptr)
		return false;
	...
	const auto previous = callback->callbacks.find(callback_key);
	if(previous != callback->callbacks.end())
		luaL_unref(callback->L, LUA_REGISTRYINDEX, previous->second);
	...
	const std::string callback_key)
{
	...
	if(owner == nullptr || owner->is_closing())
		return nullptr;
	if(owner->is_xlua2() && !owner->accepts_sdk_callbacks())
		return nullptr;
...
{
	if(callback)
		xlua_remove_callback(callback.get());
}
